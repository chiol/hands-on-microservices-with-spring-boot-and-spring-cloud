...
ACCESS_TOKEN=$(curl --request POST \
 --url 'https://dev-chiol.jp.auth0.com/oauth/token' \
 --header 'content-type: application/json' \
 --data '{"grant_type": "password", "username": "'${USER_EMAIL}'", "password":"'${USER_PASSWORD}'", "audience":"https://localhost:8443/product-composite", "scope": "openid email product:read product:write", "client_id":"'${CLIENT_ID}'", "client_secret": "'${CLIENT_SECRET}'"}' \
 -s | jq -r .access_token)
...
READER_ACCESS_TOKEN=$(curl --request POST \
 --url 'https://dev-chiol.jp.auth0.com/oauth/token' \
 --header 'content-type: application/json' \
 --data '{"grant_type": "password", "username": "'${USER_EMAIL}'", "password":"'${USER_PASSWORD}'", "audience":"https://localhost:8443/product-composite", "scope": "openid email product:read", "client_id":"'${CLIENT_ID}'", "client_secret": "'${CLIENT_SECRET}'"}' \
 -s | jq -r .access_token)
